...
Diner collection Program --- Choose one
 ...
 d:  Search the collection for specified cuisine
 k:  Search the collection for a dish whose name contains a specific word / phrase
...
def handle_commands(diners: 'list of Diner') -> 'list of Diner':
    ...
    while keep_going:
        ...
        if command == 'q':
            ...
            print (collection_to_str(collection_search_by_cuisine(diners, n)))
        elif command == 'k':
            n = input("Please enter a keyword of the dish you want to search for:   ")
            print (collection_to_str(collection_search_by_key(diners, n)))
        elif command == 'p':
...
def dish_get_info() -> Dish:
    ...
    c = float(input("Please enter the calorie of that dish:  "))
    return Dish(n, p, c)
...
def menu_calorie(dish: [Dish]) -> float:
    '''Returns the average calorie of the dish'''
    result = []
    for d in dish:
        result.append(d.calorie)
        average = sum(result) / len(result)
    return average

def menu_price(dish: [Dish]) -> float:
    '''Returns the average price of the dish'''
    result =[]
    for d in dish:
...
d1 = Dish('Escargot', 90, 100)
d2 = Dish('Snails', 100, 700)
d_list = [d1, d2]
print (menu_price(d_list))
...
def diner_to_str(eatery: Diner) -> str:
    ...
    return ("Name:     " + eatery.name + "\n" +
        ...
        "Average calories:    " + str(menu_calorie(eatery.menu)) + "\n" +
        "Average price:    $" + str(menu_price(eatery.menu)) + "\n\n")
#        "Dish:     " + eatery.dish + "\n" +
...
def collection_search_by_cuisine(diners: 'list of Diner', cuisine: str) -> [Diner]:
    '''Return a collection contatining the cuisine in diner that match the cuisine input'''
    result = []
    for d in diners:
...
def collection_search_by_key (diners: [Diner], key: str) -> [Diner]:
    '''Return a collection whose dish contains the input key'''
    result = []
    for d in diners:
        result2 = []
        for dish in d.menu:
            result2.append(dish.name)
        for name in result2:
            if key in name:
                result.append(d)
                break
